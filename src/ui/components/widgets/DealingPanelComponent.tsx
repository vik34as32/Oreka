...
export interface DealingPanelPropsType extends WidgetProps {
  startTime?: number;
  endTime?: number;
}

export interface GridProps extends DealingPanelPropsType {
...
const DealingPanelComponent = React.forwardRef<WidgetHandles, GridProps>((props: GridProps, ref): ReactElement => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (subscriptionId.current) dealingPanelDataService.current.unsubscribe(subscriptionId.current);
  ...
  const subscribe = () => {
    ...
    subscriptionId.current = dealingPanelDataService.current.getNewSubscriptionId();

    dealingPanelDataService.current.subscribe(subscriptionId.current, {
  ...
  return (
    ...
    >
      {props.dataSourceId === "dealing-panel-interval" && (
        <AddDuration
...
export default React.memo(connect(mapStateToProps, mapDispatchToProps, null, { forwardRef: true })(DealingPanelComponent));
