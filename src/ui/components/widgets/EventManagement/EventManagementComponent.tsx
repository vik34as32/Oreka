...
import CreateNewAlert from "./CreateNewAlert";
...
export interface EventManagementComponentPropsType extends WidgetProps {
  subscribedTickers: string[];
  addToFilter?: (colId: string, value: string) => void;
}

const EventManagementComponent = React.forwardRef(
  (props: EventManagementComponentPropsType, ref): ReactElement => {
    ...
    const [openCreateNewAlert, setOpenCreateNewAlert] =
    ...
    useEffect(() => {
      ...
      const updatedColStates = applyGridConfigs(
        colStates,
        props.visibleCols,
        props.groupBy,
        props.splitBy,
    ...
    useEffect(() => {
      ...
      const filterModel = Object.keys(props.columnState).reduce(
        (prev: { [colId: string]: WidgetFilter | undefined }, curr) => {
        ...
        switch (colDef.field) {
          ...
          case "delete":
            ...
            colDef.cellRendererParams = {
              handleDelete: handleTickersRemove,
            };
        }
      });
    }
    ...
    const handleTickerAdd = (ticker: string[]) => {
      ...
      setSubscribedTickers(newTickers);
      addSubscribedTickersToWidgetProps(newTickers);
      watchListService.current.subscribeToTicker(subscriptionId.current, [
        ...tickerSet,
      ]);
    };
    ...
    return (
      ...
      >
        <Box
          display={"flex"}
          sx={{ alignItems: "center",m: 0.5, ml:1, }}
        >
          <Button
            variant="contained"
            sx={{ textTransform: "none", fontFamily: "Inter",pl: 1,pr: 1, pt: 0.2, pb: 0.2, fontSize: 14 }}
            onClick={() => {
              setOpenCreateNewAlert(true);
            }}
          >
        ...
        <CreateNewAlert
          open={openCreateNewAlert}
          handleClose={() => setOpenCreateNewAlert(false)}
        ></CreateNewAlert>
      </Box>
    );
  }
...
export default React.memo(EventManagementComponent);
